...
public class ScheduleServer {
	...
	/**
	 * 任务名称里包含的服务标记，如果任务名称中没有$，默认是 BASE
	 */
	 ...
	 * TBScheduleManager类会在rewriteScheduleInfo方法中，设置他
	 */
...
//	private boolean isRegister;
	...
	public static ScheduleServer createScheduleServer(IScheduleDataManager aScheduleCenter,String aBaseTaskType,
		...
		scheduleServer.baseTaskType = aBaseTaskType;
		scheduleServer.ownSign = aOwnSign;
		scheduleServer.taskType = ScheduleUtil.getTaskTypeByBaseAndOwnSign(aBaseTaskType, aOwnSign);//组合成策略页面上真实的任务名称
		scheduleServer.ip = ScheduleUtil.getLocalIP();
		scheduleServer.hostName = ScheduleUtil.getLocalHostName();
		scheduleServer.registerTime = new Timestamp(aScheduleCenter.getSystemTime());
		scheduleServer.threadNum = aThreadNum;
		scheduleServer.heartBeatTime = null;
		scheduleServer.dealInfoDesc = "调度初始化";
		scheduleServer.version = 0;
		scheduleServer.uuid = scheduleServer.ip
				+ "$"
		...
		scheduleServer.id = Long.parseLong(s) * 100000000
				+ Math.abs(scheduleServer.uuid.hashCode() % 100000000);
